...
export default class Quiz {
  ...
  async getQuestions() {
    ...
    if (data.response_code !== 0) {
      throw new Error("No questions found for these settings. Try different options.");
    }
  ...
  getHighScores() {
    try {
      ...
      // 2 & 3. Parse JSON and return (or empty array if nothing saved)
  ...
  isHighScore() {
    ...
    // Less than 10 saved -> automatically a high score
    if (scores.length < 10) {
      return true;
    }

    // Current percentage beats the lowest saved score
    const lowest = scores[scores.length - 1].percentage;
  ...
  endQuiz() {
    ...
    // 2. Check if it's a high score — save result in variable to reuse later
    const isNewRecord = this.isHighScore();
    ...
    for (let i = 0; i < scores.length; i++) {
      ...
      leaderboardItems +=
        `<li class="leaderboard-item ${rankClass}">
          ...
          <span class="leaderboard-name">${scores[i].name}</span>
          <span class="leaderboard-score">${scores[i].percentage}%</span>
        </li>`;
    }
 
    // Build new record badge (only if high score)
    let newRecordHTML = "";
    if (isNewRecord) {
      newRecordHTML = `<div class="new-record-badge"><i class="fa-solid fa-star"></i> New High Score!</div>`;
    }
